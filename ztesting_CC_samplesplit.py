...
mc_switchpoint = args.exact_cs_switchpoint * mc_max_samples // mc_batch_size * mc_batch_size
...
pvalue_setting = ztesting(
    Sigma=Sigma, alt_type=alt_type, alt=amp
)
...
for i, seed in enumerate(seeds):
    ...
    # generate z statistics (for making e-values)
    Z = (cho @ np.random.normal(size=m)) + mu_vec
    
    # e-value details
    evalue_setting = ztesting(
        Sigma=Sigma, method='lrt', alt=amp_hat, # alt is technically a vector, should work
    )
    ...
    # e-values
    e_original = evalue_setting.e_function(Z)
    ...
    for key in CC_KEYS:
        ...
        nc_boost_set = set(CC_dict[key]['no_conf_boost_set']) | set(boost_set)
...
# save results 
results_path = f'results-amp_{args.amp}-alt_{"samplesplit"}-seeds_{seeds[0]}-{seeds[-1]}.csv'
